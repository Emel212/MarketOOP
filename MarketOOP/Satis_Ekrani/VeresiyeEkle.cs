...
namespace MarketOOP
{
    public partial class VeresiyeEkle : Form
    {
        ...
        private void VeresiyeEkle_Load(object sender, EventArgs e)
        {
           ...
           SatisID.Text=SatisEkrani.sId.ToString();
        }

        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            bool isempty = Tools.FormTools.IsEmpty(panel1);
            if (isempty)
            {
        ...
        {
            if(SatisID.Text==string.Empty || SatisID.Text=="0")
            {
                MessageBox.Show("Kayıt için satış işlemi yapılmalı");
            }
            else
            {
                panel1.Enabled = true;
